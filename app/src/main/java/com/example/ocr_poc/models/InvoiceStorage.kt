...
data class Invoice(
    val entities: List<TextEntity> // Removed imageUri
)

object InvoiceStorage {
    private val Context.dataStore by preferencesDataStore(name = "invoices")

    private val INVOICES_KEY = stringSetPreferencesKey("invoices")

    // Add an invoice
    suspend fun addInvoice(context: Context, invoice: Invoice) {
        val gson = Gson()
        val invoiceString = gson.toJson(invoice)

        context.dataStore.edit { preferences ->
            val currentInvoices = preferences[INVOICES_KEY] ?: emptySet()
            if (!currentInvoices.contains(invoiceString)) {
                preferences[INVOICES_KEY] = currentInvoices + invoiceString
            }
        }
    }

    // Update an invoice
    suspend fun updateInvoice(context: Context, invoiceToUpdate: Invoice, updatedEntities: List<TextEntity>) {
        val gson = Gson()
        val invoiceToUpdateString = gson.toJson(invoiceToUpdate)

        context.dataStore.edit { preferences ->
            val currentInvoices = preferences[INVOICES_KEY] ?: emptySet()
            val updatedInvoice = invoiceToUpdate.copy(entities = updatedEntities)
            val updatedInvoiceString = gson.toJson(updatedInvoice)

            if (currentInvoices.contains(invoiceToUpdateString)) {
                val newInvoices = currentInvoices - invoiceToUpdateString + updatedInvoiceString
                preferences[INVOICES_KEY] = newInvoices
            }
        }
    }
    ...
    fun getInvoices(context: Context): Flow<List<Invoice>> {
        ...
        return context.dataStore.data.map { preferences ->
            val invoicesSet = preferences[INVOICES_KEY] ?: emptySet()
            invoicesSet.map { invoiceString ->
                gson.fromJson(invoiceString, Invoice::class.java)
            }
        }
    }
