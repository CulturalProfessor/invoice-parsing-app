...
def is_item_token(token):
    return token.lower() in brands or token.lower() in item_keywords
...
def tag_sections(tokens):
    ...
    for i, token in enumerate(tokens):
        ...
        if re.match(r"^\d{7,}$", clean_token):
            tag = "B-INVOICE"
        elif re.match(r"^\d{2}/\d{2}/\d{4}$", clean_token) or re.match(
            r"^\d{4}-\d{2}-\d{2}$", clean_token
        ):
            # explicitly tagging date as O
            tag = "O"
            ...
            tag = "B-TAX" if previous_tag == "O" else "I-TAX"
        elif re.match(r"^\d+(\.\d{1,2})?$", clean_token):
            tag = "B-PRICE" if previous_tag == "O" else "I-PRICE"
        elif re.match(r"(Model|Product)\s?No[:\-]?", clean_token, re.IGNORECASE):
            tag = "B-ITEM"
        else:
            if (
                ...
                tag = "B-CUSTOMER" if previous_tag == "O" else "I-CUSTOMER"
            elif is_item_token(clean_token):
                tag = "B-ITEM" if previous_tag == "O" else "I-ITEM"
            elif clean_token[0].isupper() and clean_token not in known_invoice_terms:
                tag = "B-VENDOR" if previous_tag == "O" else "I-VENDOR"
...
def process():
    input_csv_paths = ["./extracted_texts.csv"]
...
if __name__ == "__main__":
    ...
    tag_counts = Counter(tag for seq in processed_data for tag in seq["ner_tags"])
    print("Tag Counts:")
